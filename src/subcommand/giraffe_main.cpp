    ...
    while (true) {
        ...
        switch (c)
        {
            case 'Z':
                ...
                registry.provide("safari GBZ", optarg);
            ...
            case 'H':
                ...
                registry.provide("safari GBWT", optarg);
                break;
            ...
            case 'd':
                ...
                registry.provide("safari Distance Index", optarg);
                break;
            ...
            case 'o':
                {
                    ...
                    if (output_formats.find(output_format) == output_formats.end()) {
                        std::cerr << "error: [vg safari] Invalid output format: " << optarg << std::endl;
                        std::exit(1);
                    }
                }
                break;
            ...
            case 'b':
                ...
                if (param_preset == "fast" ) {
                    ...
                    std::cerr << "error: [vg safari] invalid parameter preset: " << optarg << std:: endl;
                }
                break;
            ...
            case 'c':
                {
                    ...
                    if (cap <= 0) {
                        cerr << "error: [vg safari] Hit cap (" << cap << ") must be a positive integer" << endl;
                        exit(1);
                    }
                    hit_cap = cap;
                }
            ...
            case 'C':
                {
                    auto cap = parse<Range<size_t>>(optarg);
                    if (cap <= 0) {
                        cerr << "error: [vg safari] Hard hit cap (" << cap << ") must be a positive integer" << endl;
                        exit(1);
                    }
                    hard_hit_cap = cap;
                }
            ...
            case 'D':
                {
                    ...
                    if (limit <= 0) {
                        cerr << "error: [vg safari] Distance limit (" << limit << ") must be a positive integer" << endl;
                        exit(1);
                    }
            ...
            case 'e':
                {
                    ...
                    if (extensions <= 0) {
                        cerr << "error: [vg safari] Number of extensions (" << extensions << ") must be a positive integer" << endl;
                        exit(1);
                    }
            ...
            case 'a':
                {
                    ...
                    if (alignments <= 0) {
                        cerr << "error: [vg safari] Number of alignments (" << alignments << ") must be a positive integer" << endl;
                        exit(1);
                    }
            ...
            case 's':
                {
                    ...
                    if (score < 0) {
                        cerr << "error: [vg safari] Cluster score threshold (" << score << ") must be positive" << endl;
                        exit(1);
                    }
                    cluster_score = score;
                }
            ...
            case 'S':
                {
                    auto score = parse<Range<double>>(optarg);
                    if (score < 0) {
                        cerr << "error: [vg safari] Second best cluster score threshold (" << score << ") must be positive" << endl;
                        exit(1);
                    }
            ...
            case 'u':
                {
                    auto score = parse<Range<double>>(optarg);
                    if (score < 0) {
                        cerr << "error: [vg safari] Cluster coverage threshold (" << score << ") must be positive" << endl;
                        exit(1);
                    }
            ...
            case 'U':
                {
                    ...
                    if (maxmin <= 0) {
                        cerr << "error: [vg safari] Maximum unique minimizer (" << maxmin << ") must be a positive integer" << endl;
                        exit(1);
                    }
            ...
            case 'v':
                {
                    ...
                    if (score < 0) {
                        cerr << "error: [vg safari] Extension score threshold (" << score << ") must be positive" << endl;
                        exit(1);
                    }
            ...
            case 'w':
                {
                    auto score = parse<Range<double>>(optarg);
                    if (score < 0) {
                        cerr << "error: [vg safari] Extension set score threshold (" << score << ") must be positive" << endl;
                        exit(1);
                    }
                    extension_set = score;
                }
            ...
            case 'r':
                {
                    ...
                    if (rescue_attempts < 0) {
                        cerr << "error: [vg safari] Rescue attempts must be positive" << endl;
                        exit(1);
                    }
                }
                break;
            ...
            case 'A':
                {
                    ...
                    if (iter == rescue_algorithms.end()) {
                        std::cerr << "error: [vg safari] Invalid rescue algorithm: " << optarg << std::endl;
                        std::exit(1);
                    }
           ...
           case 'j':
                posterior_threshold = parse<Range<double>>(optarg);
                break;
...
if (deam3pfreqE == "" || deam5pfreqE == ""){throw runtime_error("[safari] Must provide damage matrix estimate files");}
    ...
    if (!ref_paths_name.empty() && !hts_output) {
        cerr << "warning:[vg safari] Reference path file (--ref-paths) is only used when output format (-o) is SAM, BAM, or CRAM." << endl;
        ref_paths_name = "";
    }
    ...
    if ((forced_mean && ! forced_stdev) || (!forced_mean && forced_stdev)) {
        cerr << "warning:[vg safari] Both a mean and standard deviation must be specified for the fragment length distribution" << endl;
    ...
    if ((forced_mean || forced_stdev || forced_rescue_attempts) && (!paired)) {
        cerr << "warning:[vg safari] Attempting to set paired-end parameters but running in single-end mode" << endl;
    }
    ...
    unordered_map<string, vector<string>> indexes_and_extensions = {
        {"safari GBZ", {"safari.gbz", "gbz"}},
        {"XG", {"xg"}},
        {"safari GBWT", {"gbwt"}},
        {"GBWTGraph", {"gg"}},
        {"safari Distance Index", {"dist"}},
        {"Minimizers", {"min"}}
    };
    ...
    auto index_targets = VGIndexes::get_default_safari_indexes();
    ...
    auto distance_index = vg::io::VPKG::load_one<SnarlDistanceIndex>(registry.require("safari Distance Index").at(0));
    if (show_progress) {
    ...
    if (!report_name.empty()) {
        ...
        if (!report) {
            // Make sure it worked
            cerr << "error[vg safari]: Could not open report file " << report_name << endl;
            exit(1);
        }
    ...
    for_each_combo([&]() {
        ...
        if (show_progress && show_work) {
            cerr << "--posterior-threshold " << endl;
        }
        minimizer_mapper.posterior_threshold = posterior_threshold;
        ...
        for (auto& perf_fd : perf_fds) {
            if (perf_fd > 0) {
                ...
                if (read(perf_fd, &thread_instructions, sizeof(long long)) != sizeof(long long)) {
                    ...
                    cerr << "warning:[vg safari] Could not count CPU instructions executed" << endl;
                    thread_instructions = 0;
                }
                if (close(perf_fd)) {
                    int problem = errno;
                    cerr << "warning:[vg safari] Error closing perf event instruction counter: " << strerror(problem) << endl;
                }
...
// Register subcommand
static Subcommand vg_safari("safari", "fast haplotype-aware short read alignment", PIPELINE, 6, main_safari);
