...
int main_minimizer(int argc, char** argv) {
    ...
    if (load_index.empty()) {
        index = std::make_unique<gbwtgraph::DefaultMinimizerIndex>(IndexingParameters::minimizer_k, 
            ...
            use_syncmers);
        rymer_index = std::make_unique<gbwtgraph::DefaultMinimizerIndex>(IndexingParameters::minimizer_k,
            (use_syncmers ? IndexingParameters::minimizer_s : IndexingParameters::minimizer_w),
            use_syncmers);
    } else {
        if (progress) {
  ...
  if (distance_name.empty()) {
    gbwtgraph::index_haplotypes(gbz->graph, false, *index, [](const pos_t&) -> gbwtgraph::payload_type {
        return MIPayload::NO_CODE;
    }, IndexingParameters::minimizer_k);
} else {
    gbwtgraph::index_haplotypes(gbz->graph, false, *index, [&](const pos_t& pos) -> gbwtgraph::payload_type {
    ...
    if (progress) {
        ...
        std::cerr << "Hash table size: " << index->capacity() << std::endl;
    }
