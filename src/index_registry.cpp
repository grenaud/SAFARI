...
int IndexingParameters::safari_gbwt_downsample = gbwtgraph::LOCAL_HAPLOTYPES_DEFAULT_N;
...
#pragma omp critical
    ...
    // safari will prefer to use a downsampled haplotype GBWT if possible
    registry.register_recipe({"safari GBWT"}, {"GBWT", "XG"},
                             [](const vector<const IndexFile*>& inputs,
                                const IndexingPlan* plan,
                                AliasGraph& alias_graph,
                                const IndexGroup& constructing) {
        ...
        size_t threshold = IndexingParameters::safari_gbwt_downsample * IndexingParameters::downsample_threshold;
        ...
        if (downsample) {
            ...
            function<void(void)> exec = [&]() {
                gbwt::GBWT cover = gbwtgraph::local_haplotypes(*xg_index, *gbwt_index,
                                                               IndexingParameters::safari_gbwt_downsample,
        ...
        if (!success) {
            ...
            throw RewindPlanException("[IndexRegistry]: Exceeded GBWT insert buffer size, expanding and reattempting.", {"safari GBWT"});
        }
    ...
    registry.register_recipe({"safari GBWT"}, {"XG"},
                             [](const vector<const IndexFile*>& inputs,
                                const IndexingPlan* plan,
                                AliasGraph& alias_graph,
                                const IndexGroup& constructing) {
    ...
    // TODO: disabling so that we can distinguish safari graphs that may have
...
//    });
    ...
    registry.register_recipe({"safari Distance Index"}, {"safari GBZ"},
                             [make_distance_index](const vector<const IndexFile*>& inputs,
                                 const IndexingPlan* plan,
                                 AliasGraph& alias_graph,
                                 const IndexGroup& constructing) {
        if (IndexingParameters::verbosity != IndexingParameters::None) {
            cerr << "[IndexRegistry]: Constructing distance index for safari." << endl;
        }
    ...
    registry.register_recipe({"safari GBZ"}, {"Reference GFA w/ Haplotypes"},
                             [](const vector<const IndexFile*>& inputs,
                                const IndexingPlan* plan,
                                AliasGraph& alias_graph,
                                const IndexGroup& constructing) {
        if (IndexingParameters::verbosity != IndexingParameters::None) {
            cerr << "[IndexRegistry]: Combining safari GBWT and GBWTGraph into GBZ." << endl;
        }
    ...
    registry.register_recipe({"safari GBZ"}, {"GBWTGraph", "safari GBWT"},
                             [](const vector<const IndexFile*>& inputs,
                                const IndexingPlan* plan,
                                AliasGraph& alias_graph,
                                const IndexGroup& constructing) {
    ...
    registry.register_recipe({"Rymers"}, {"safari Distance Index", "safari GBZ"},
                             [](const vector<const IndexFile*>& inputs,
                                const IndexingPlan* plan,
                                AliasGraph& alias_graph,
                                const IndexGroup& constructing) {
        ...
        gbwtgraph::index_haplotypes(gbz->graph, true, rymers, [&](const pos_t& pos) -> gbwtgraph::payload_type {
    ...
    registry.register_recipe({"Minimizers"}, {"safari Distance Index", "safari GBZ"},
                             [](const vector<const IndexFile*>& inputs,
                                const IndexingPlan* plan,
                                AliasGraph& alias_graph,
                                const IndexGroup& constructing) {
        ...
        gbwtgraph::index_haplotypes(gbz->graph, false, minimizers, [&](const pos_t& pos) -> gbwtgraph::payload_type {
            return MIPayload::encode(get_minimizer_distances(*distance_index, pos));
        });
...
vector<IndexName> VGIndexes::get_default_safari_indexes() {
    vector<IndexName> indexes{
        "safari Distance Index",
        "safari GBZ",
        "Minimizers",
        "Rymers"
    };
