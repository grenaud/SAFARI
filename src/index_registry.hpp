...
struct IndexingParameters {
    ...
    // the number of haplotypes to downsample to in safari's GBWT [64]
    static int safari_gbwt_downsample;
    // sample subpaths of this length (in nodes) [4]
    static int downsample_context_length;
    // augment the existing GBWT instead of downsampling it if the number of haplotypes is < this * safari_gbwt_downsample [3]
...
struct VGIndexes {
    ...
    /// A list of the identifiers of the default indexes to run vg safari
    static vector<IndexName> get_default_safari_indexes();
};
