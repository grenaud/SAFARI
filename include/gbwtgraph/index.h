                 ...
                 const std::function<payload_type(const pos_t&)>& get_payload, unsigned int k=21)
{
  ...
  auto flush_cache = [&](int thread_id)
  {
    ...
    #pragma omp critical (minimizer_index)
    {
      for(size_t i = 0; i < current_cache.size(); i++)
      {
        ...
        //std::cerr << "KEY INSERTED INTO KMER TABLE KEY: " << current_cache[i].first.key.get_key() << std::endl;
        ...
        //current_cache[i].first.original_kmer_key = current_cache[i].first.key;
        //current_cache[i].first.original_kmer_hash = current_cache[i].first.key.hash();
        //std::cerr << "IN INDEX, KMER KEY: " << current_cache[i].first.key.get_key() << std::endl;
        ...
        //std::cerr << "IN INDEX, CONVERTED KEY: " << current_cache[i].first.key.get_key() << std::endl;
        //std::cerr << "KEY ADDED TO RYMER PAYLOAD: " << current_cache[i].first.original_kmer_key.get_key() << std::endl;
  ...
  auto find_minimizers = [&](const std::vector<handle_t>& traversal, const std::string& seq)
  {
   ...
   if(rymer){
    //for (auto &m : minimizers){std::cerr << "IN INDEX, ADDING TO RYMER INDEX: " << m.key.decode_rymer(index.k()) << std::endl;}
   }

   else{
    //for (auto &m : minimizers){std::cerr << "IN INDEX, ADDING TO MINIMIZER INDEX: " << m.key.decode(index.k()) << std::endl;}
       }
